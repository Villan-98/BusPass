...
route.get("/", (req, res) => {
  if (req.isAuthenticated()) {
    if (req.user.role === "Transport Head") {
      ...
      application.applicationByClg(req.query).then(data => {
        res.status(200).send({
          success: true,
          data: data,
          code: 200
...
route.post("/", upload.array("photo", 3), (r, s) => {
  ...
  application
    .newApplication(r.body)
    .then(() => {
      application.getStatus(r.body).then(data => {
        console.log(data);
  ...
  client.convertUrlToFile(
    ...
    function(err, fileName) {
      ...
      fs.readFile(fileName, (err, data) => {
        ...
        //console.log(temp)
        fs.writeFile(
          "public/assets/registration_form/" + fileName,
          data,
          err => {
            if (err) {
              console.log(err);
